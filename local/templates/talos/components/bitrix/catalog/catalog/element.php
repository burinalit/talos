...
<?$ElementID = $APPLICATION->IncludeComponent(
	...
	array(
        ...
        "IBLOCK_ID_DOPINFO" => $arParams['IBLOCK_ID_DOPINFO']
	),
	$component
);?><?
if (0 < $ElementID)
...
if($arParams['DETAIL_PROPS_ANALOG']) {
    ...
    $db_props = CIBlockElement::GetProperty($arParams['IBLOCK_ID'], $ElementID, array("sort" => "asc"), Array("ACTIVE"=>"Y"));
    $arCodeProp = array();
    while($prop_recom = $db_props->Fetch())
    {
        $arCodeProp[$prop_recom['CODE']] = $prop_recom;
    }
    ...
    foreach($arParams['DETAIL_PROPS_ANALOG'] as $det_props) {
        if($det_props != '' && isset($arCodeProp[$det_props])){
            $arFilterProps['PROPERTY_'.$det_props] = $arCodeProp[$det_props]['VALUE'];
        }

    }

    $GLOBALS['arrFilterRecomend'] = $arFilterProps;
}
?>
<div class="bg-silver">
<div class="container-fluid">
    <?if(count($GLOBALS['arrFilterRecomend'])> 0):?>
        <?$APPLICATION->IncludeComponent(
...
<?$APPLICATION->IncludeComponent(
    ...
    Array(
        ...
        "ADDITIONAL_PICT_PROP_5" => "EMARKET_PHOTO",
        ...
        'MESS_BTN_COMPARE' => $arParams['MESS_BTN_COMPARE'],
        'MESS_NOT_AVAILABLE' => $arParams['MESS_NOT_AVAILABLE'],
        "MESS_BTN_DETAIL" => $arParams['MESS_BTN_BUY'],
