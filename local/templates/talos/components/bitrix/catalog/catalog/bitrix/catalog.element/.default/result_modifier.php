...
//Obtain directories values ??used
...
{
	if(($arParams['BLOG_HLBLOCK_PROP_CODE'] == $code) || ($arParams['BLOG_HLBLOCK_CR_PROP_CODE'] == $code))
		continue;
...
foreach ($reqParams as $params)
{
	...
	{
		$arResult['HL_PROP_LIST'][$params['HLB']['TABLE_NAME']][$arEnum['UF_XML_ID']] = $arEnum;
		$arResult['HL_PROP_LIST'][$params['HLB']['TABLE_NAME']][$arEnum['ID']] = $arEnum;
	}
}
//get property
if($arParams['GROUPS_OF_PROP_IBLOCK_ID'] > 0)
{
	$arResult['PROPERTY_ITEMS'] = array();

	$arSelect = Array("ID", "IBLOCK_ID", "NAME", "CODE");
	$arFilter = Array("IBLOCK_ID"=>IntVal($arParams['GROUPS_OF_PROP_IBLOCK_ID']), "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
	$res = CIBlockElement::GetList(Array('PROPERTY_property_group' => 'ASC'), $arFilter, false, false, $arSelect);
	while($ob = $res->GetNextElement())
	{
		$arFields = $ob->GetFields();
		$arProps = $ob->GetProperties();

		$arResult['PROPERTY_ITEMS'][$arFields['ID']] = $arFields;
		$arResult['PROPERTY_ITEMS'][$arFields['ID']]['PROPERTIES'] = $arProps;
	}
}
