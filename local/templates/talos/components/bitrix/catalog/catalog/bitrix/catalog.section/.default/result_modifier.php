...
if( $arResult['ITEMS'][0]['PROPERTIES']['EMARKET_BRAND']['USER_TYPE_SETTINGS']['TABLE_NAME']
    && CModule::IncludeModule("highloadblock")
)
{    
	//Укажем вручную Id Hightload инфоблока с виджетами
	$table_name = $arResult['ITEMS'][0]['PROPERTIES']['EMARKET_BRAND']['USER_TYPE_SETTINGS']['TABLE_NAME'];
	...
	$resh = $entity_data_class::getList(array(
		'select' =>array(
			'ID',
			'UF_FILE',
			'UF_XML_ID',
	...
	while($arResultHL = $resh->fetch())
	{
		$arResult['BRANDS'][$arResultHL['UF_XML_ID']] = $arResultHL;
	}
    endif;
...
$GLOBALS['peremen'] = count($arResult['ITEMS']);
